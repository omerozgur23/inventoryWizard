...
public interface OrderDetailsRepository extends JpaRepository<OrderDetails, UUID> {
	...
	@Query("SELECT SUM(od.totalPrice) FROM OrderDetails od WHERE od.status = true")
	double getTotalSale();

	@Query("SELECT SUM(od.quantity * p.purchasePrice) AS totalSalesCost FROM OrderDetails od JOIN od.product p WHERE od.status = true")
